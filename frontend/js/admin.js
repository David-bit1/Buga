...
const handleMovieSubmit = async (event) => {
    ...
    const payload = {
        ...
        genres: movieGenres.value.trim().split(',').map(name => name.trim()).filter(Boolean).map(name => ({ name })),
        rating: movieRating.value.trim(),
        cast: movieCast.value.trim().split(',').map(name => name.trim()).filter(Boolean).map(name => ({ name })),
