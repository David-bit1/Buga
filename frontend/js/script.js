...
const buildMediaHref = (movieId, mediaType = 'movie') => {
    const movie = featuredMoviesCache.find(m => String(m.id) === String(movieId));
    // Always prefer tmdb_id for the detail page link
    const idToUse = movie?.tmdb_id || movie?.id || movieId;
